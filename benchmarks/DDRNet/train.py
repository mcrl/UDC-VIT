...
def set_dirs(opt):
    ...
    opt['log_dir'] = f"{opt['log_dir']}/{opt['task']}"
    opt['log_dir'] += f"_run1"
    ...
    dist.broadcast_object_list([opt['log_dir']], 0)
...
'''
# --------------------------------------------
# training code for DDRNet
# --------------------------------------------
'''


def main(json_path='options/DDRNet/train_DDRNet.json'):
    ...
    set_dirs(opt) 
    summarywriter = SummaryWriter(log_dir=opt['log_dir']) if opt['rank'] == 0 else None 
            ...
            if current_step % opt['train']['checkpoint_test'] == 0:# and opt['rank'] == 0:
                ...
                for idx, test_data in enumerate(test_loader):
                    ...
                    if gt is not None:
                        ...
                        print('[{:04d}-{:02d}-{:02d} {:02d}:{:02d}:{:02d}] Testing {:20s} ({:2d}/{}) - PSNR: {:.2f} dB; SSIM: {:.4f}; '
                                    'PSNR_Y: {:.2f} dB; SSIM_Y: {:.4f}'.
                                    format(cur_time.year, cur_time.month, cur_time.day, 
                                           cur_time.hour, cur_time.minute, cur_time.second,  
                                           folder[0], idx+1, len(test_loader), psnr, ssim, psnr_y, ssim_y))
                ...
                if gt is not None:
                    ...
                    ave_psnr = mp_all_reduce_mean(test_results['psnr'])
                    ave_ssim = mp_all_reduce_mean(test_results['ssim'])
                    ave_psnr_y = mp_all_reduce_mean(test_results['psnr_y'])
                    ave_ssim_y = mp_all_reduce_mean(test_results['ssim_y'])
                    ...
                    if opt['rank'] == 0:    
                        ...
                        summarywriter.add_scalar("validation/PSNR", ave_psnr, current_step)
                        summarywriter.add_scalar("validation/SSIM", ave_ssim, current_step)
                        cur_time = datetime.datetime.now()
                        print('[{:04d}-{:02d}-{:02d} {:02d}:{:02d}:{:02d}] Testing AVG - PSNR: {:.2f} dB; SSIM: {:.4f}; '
                                    'PSNR_Y: {:.2f} dB; SSIM_Y: {:.4f}'.
                                    format(cur_time.year, cur_time.month, cur_time.day,
                                           cur_time.hour, cur_time.minute, cur_time.second, 
                                           ave_psnr, ave_ssim, ave_psnr_y, ave_ssim_y))
