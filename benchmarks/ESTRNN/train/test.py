...
def _test_torch(para, logger, model, ds_type):
    ...
    dataset_path = join(para.data_root,  '{}_{}'.format(para.dataset, para.ds_config), ds_type)
    ...
    for seq in seqs:
        ...
        suffix = 'npy' if para.data_format == 'RGB' else 'tiff'
        ...
        while True:
            ...
            for frame_idx in range(start, end):
                ...
                dirname, fname = '{:04d}'.format(int(seq)), '{:04d}'.format(int(frame_idx))
                
                blur_img_path = os.path.join(dataset_path, 'Input', dirname, fname+'.npy')
                sharp_img_path = os.path.join(dataset_path, 'GT', dirname, fname+'.npy')
                # print(blur_img_path,sharp_img_path)
                ...
                blur_img = np.load(blur_img_path).transpose(2, 0, 1)[np.newaxis, ...]
                gt_img = np.load(sharp_img_path)
            ...
            for frame_idx in range(para.past_frames, end - start - para.future_frames):
                ...
                deblur_img_path = join(save_dir, '{:04d}'.format(frame_idx + start )) 
                ...
                if deblur_img_path not in results_register:
                    ...
                    psnr_result = psnr_calculate(deblur_img, gt_img, val_range=val_range)
                    ssim_result = ssim_calculate(deblur_img, gt_img, val_range=val_range)
                    PSNR.update(psnr_result)
                    SSIM.update(ssim_result)

                img_name = f"{deblur_img_path}_{psnr_result:.2f}_{ssim_result:.4f}.png" 

                deblur_img_pil = Image.fromarray(deblur_img) 
                deblur_img_pil.save(img_name)
