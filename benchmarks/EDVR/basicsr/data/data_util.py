...
def read_npy_seq(path, require_mod_crop=False, scale=1): # kyusu
    """Read a sequence of images from a given folder path.
    ...
    if isinstance(path, list):
        ...
        img_paths = sorted(list(scandir(path, full_path=True)))
    imgs = [np.load(v).astype(np.float32) / 255. for v in img_paths]
    if require_mod_crop:
        imgs = [mod_crop(img, scale) for img in imgs]
    imgs = img2tensor(imgs, bgr2rgb=False, float32=True) # kyusu
    imgs = torch.stack(imgs, dim=0)
    return imgs
