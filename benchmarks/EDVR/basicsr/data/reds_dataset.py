...
class REDSDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.gt_root, self.lq_root = Path(opt['dataroot_gt']), Path(
            opt['dataroot_lq'])
        self.flow_root = Path(
            opt['dataroot_flow']) if opt['dataroot_flow'] is not None else None # None이 들어오네
        ...
        with open(opt['meta_info_file'], 'r') as fin:
            for line in fin:
                ...
                self.keys.extend(
                    [f'{folder}/{i:04d}' for i in range(int(frame_num))])
    ...
    def __getitem__(self, index):
        ...
        # each clip has 100 frames starting from 0 to 179
        while (start_frame_idx < 0) or (end_frame_idx > 179):
        ...
        if self.is_lmdb:
            ...
            img_gt_path = self.gt_root / clip_name / f'{frame_name}.npy'
        ...
        for neighbor in neighbor_list:
            if self.is_lmdb:
                ...
                img_lq_path = self.lq_root / clip_name / f'{neighbor:04d}.npy'
        ...
        if self.flow_root is not None:
            ...
            for i in range(self.num_half_frames, 0, -1):
                if self.is_lmdb:
                    ...
                    flow_path = (
                        self.flow_root / clip_name / f'{frame_name}_p{i}.npy')
            ...
            for i in range(1, self.num_half_frames + 1):
                if self.is_lmdb:
                    ...
                    flow_path = (
                        self.flow_root / clip_name / f'{frame_name}_n{i}.npy')
                img_bytes = self.file_client.get(flow_path, 'flow')
                cat_flow = imfrombytes(
                    img_bytes, flag='grayscale',
                    float32=False)  # uint8, [0, 255]
