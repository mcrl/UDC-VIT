...
from basicsr.data.data_util import (duf_downsample, generate_frame_indices,
                                    read_img_seq, read_npy_seq)
from basicsr.utils import get_root_logger, scandir, img2tensor
import time

from basicsr.data.transforms import paired_random_crop, paired_crop
import numpy as np
import torchvision
...
class VideoTestDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.cache_data:
            ...
            img_path_gt = self.imgs_gt[folder][idx]
            imgs_lq = []
            for img_path_lq in img_paths_lq:
                img_lq = np.load(img_path_lq)
                img_lq = img_lq.astype(np.float32) / 255.0
                imgs_lq.append(img_lq)
            img_gt = np.load(img_path_gt)
            img_gt = img_gt.astype(np.float32) / 255.0

        # add the line below (due to OOM)
        if inference == 'false' or inference=='False':
            img_gt, imgs_lq = paired_random_crop(img_gt, imgs_lq, gt_size, scale, gt_path=None)
        elif inference == 'left':
            img_gt, imgs_lq = paired_crop(img_gt, imgs_lq, gt_path=None, where='left')
        elif inference == 'right':
            img_gt, imgs_lq = paired_crop(img_gt, imgs_lq, gt_path=None, where='right')
        
        imgs_lq.append(img_gt)
        img_results = img2tensor(imgs_lq, bgr2rgb=False, float32=True)
        imgs_lq = torch.stack(img_results[0:-1], dim=0)
