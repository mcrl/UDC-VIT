    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        def _save_image(img_npy, img_path, max_pxl=1023.0, dng_info=None):
            if img_npy.shape[2] == 3:
                ...
                # mmcv.imwrite(img_npy[:, :, ::-1], img_path) 
            else:
        ...
        for idx, val_data in enumerate(dataloader):
            img_name_tmp = osp.splitext(osp.basename(val_data["lq_path"][0]))[0]
            
       
            lq_path = val_data["lq_path"][0]  # Assuming there's only one path per entry
            clip_name = osp.basename(osp.dirname(lq_path))  # Extract clip_name (e.g., '0071')
            frame_name = osp.splitext(osp.basename(lq_path))[0]  # Extract frame_name without extension (e.g., '0000')
            ...
            psnr_value = image_metric.get('psnr', 0) 
            ssim_value = image_metric.get('ssim', 0) 
            
            metric_message = (
                f"{img_name_tmp}_{image_metric.get('psnr', 0)}_{image_metric.get('ssim',0)}"
            )
            ...
            if save_img:
                img_name = f"{clip_name}/{frame_name}_{psnr_value:.2f}_{ssim_value:.4f}.png" 
                ...
                if self.opt["is_train"]:
                    ...
                    if self.opt["val"].get("suffix"):
                        ...
                        # )
                        imgdir = osp.join(self.opt["path"]["visualization"], dataset_name)
                        os.makedirs(imgdir, exist_ok=True)
                        save_img_path = osp.join(imgdir, img_name)
                    _save_image(
                        sr_img, save_img_path, max_pxl=max_pxl, dng_info=dng_info
                    )
