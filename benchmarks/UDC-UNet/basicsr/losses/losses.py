...
class MSE_FFT(nn.Module): 
    ...
    def forward(self, arg_input, arg_gt):
        ...
        loss_mse = self.clamp(_input, gt)  
        loss_fft_abs = self.fft_abs(_input, gt) 
        loss_fft_angle = self.fft_angle(_input, gt) 
        loss_total = loss_mse + loss_fft_abs + loss_fft_angle
        return loss_total


@LOSS_REGISTRY.register()
class NO_MSE(nn.Module):  
    def __init__(self):
