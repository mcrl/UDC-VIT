...
void LibAvEncoder::initAudioInCodec(VideoOptions const *options, StreamInfo const &info)
{
	...
	if (ret < 0)
	{
		...
		throw std::runtime_error("libav: cannot open " + options->audio_source + " input device " +
...
LibAvEncoder::LibAvEncoder(VideoOptions const *options, StreamInfo const &info, int world_rank)
	  ...
	  audio_samples_(0), in_fmt_ctx_(nullptr), out_fmt_ctx_(nullptr), output_file_(options->output)
{
	...
	if (world_rank == 0)
	{
		output_file_ = result_dir + std::to_string(dir_name) + '/' + options_->output + "_" +
					   std::to_string(world_rank) + options_->output_format;
	}
