...
void Output::OutputReady(void *mem, size_t size, int64_t timestamp_us, bool keyframe, int num)
{
	...
	outputBuffer(mem, size, last_timestamp_, flags, num);
...
void Output::outputBuffer(void *mem, size_t size, int64_t timestamp_us, uint32_t flags, int num)
{
	// Supply this so that a vanilla Output gives you an object that outputs no buffers.
}

Output *Output::Create(VideoOptions const *options, int num)
{
	...
	if (num == 1)
	{
		if (strncmp(options->output_format.c_str(), "udp://", 6) == 0 || strncmp(options->output_format.c_str(), "tcp://", 6) == 0)
		{
