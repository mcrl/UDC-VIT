...
{
public:
	static Output *Create(VideoOptions const *options, int num);

	Output(VideoOptions const *options);
	virtual ~Output();
	virtual void Signal(); // a derived class might redefine what this means
	void OutputReady(void *mem, size_t size, int64_t timestamp_us, bool keyframe, int num);
...
protected:
	...
	virtual void outputBuffer(void *mem, size_t size, int64_t timestamp_us, uint32_t flags, int num);
