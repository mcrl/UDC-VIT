...
void RPiCamApp::OpenCamera(int world_rank = 0)
{
	// Make a preview window.
	global_world_rank = world_rank;
	...
	std::string const &cam_id = cameras[world_rank]->id();
...
void RPiCamApp::StartCamera()
{
	...
	if (!controls_.get(controls::HdrMode) && (options_->hdr == "auto" || options_->hdr == "single-exp"))
...
{
	std::string filename = "log_" + std::to_string(global_world_rank) + ".txt";
	...
	int dir_name = i - 3;
	std::ofstream ofile(result_dir + std::to_string(dir_name) + "/" + filename);
	while (true)
	{
		PreviewItem item;
		while (!item.stream)
	...
	LOG(1, "log file saved as log.txt");
	ofile.close();
}
