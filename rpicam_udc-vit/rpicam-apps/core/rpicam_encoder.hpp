...
public:
	...
	void StartEncoder(int world_rank)
	{
		createEncoder(world_rank);
...
protected:
	virtual void createEncoder(int world_rank)
	{
		StreamInfo info;
		VideoStream(&info);
		if (!info.width || !info.height || !info.stride)
			throw std::runtime_error("video steam is not configured");
		encoder_ = std::unique_ptr<Encoder>(Encoder::Create(GetOptions(), info,world_rank));
	}
