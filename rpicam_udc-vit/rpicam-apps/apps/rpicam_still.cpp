					   ...
					   std::string const &filename)
{
	...
	std::ofstream os("myfile.bin", std::ios::binary); 
 	...
 	os.write(reinterpret_cast<const char*>(&mem), sizeof(mem)); 
...
static void save_images(RPiCamStillApp &app, CompletedRequestPtr &payload, int world_rank)
{
	StillOptions *options = app.GetOptions();
	std::string filename = generate_filename(options);
	filename = filename.substr(0, filename.rfind('.')) + "_"+ std::to_string(world_rank) + ".png";
...
static void event_loop(RPiCamStillApp &app,int world_rank)
{
	StillOptions const *options = app.GetOptions();
	...
	for (unsigned int count = 0;; count++)
	{
		...
		else if (app.StillStream() && want_capture)
		{
			...
			save_images(app, completed_request,world_rank);
